...
<%Connection con1 = null;try{Class.forName("com.mysql.jdbc.Driver");con1= DriverManager.getConnection("jdbc:mysql://localhost:3306/movierec?zeroDateTimeBehavior=convertToNull","root","");}catch(Exception e){e.printStackTrace();}%>

<h2>Table Name - JobApplicationPage</h2><table border="1"><tr>

<%Statement stdisp=con1.createStatement();ResultSet aResultset = stdisp.executeQuery("select COLUMN_NAME, DATA_TYPE from INFORMATION_SCHEMA.COLUMNS IC where TABLE_NAME = 'JobApplicationPage' and table_schema= 'movierec'");
...
<%Statement stdisp_chk0=con1.createStatement();ResultSet aResultset_chk0 = stdisp_chk0.executeQuery("select COLUMN_NAME, DATA_TYPE from INFORMATION_SCHEMA.COLUMNS IC where TABLE_NAME = 'JobApplicationPage_fields_20'  and table_schema= 'movierec'");
...
<%Statement stdisp_chk1=con1.createStatement();ResultSet aResultset_chk1 = stdisp_chk1.executeQuery("select COLUMN_NAME, DATA_TYPE from INFORMATION_SCHEMA.COLUMNS IC where TABLE_NAME = 'JobApplicationPage_city_21'  and table_schema= 'movierec'");
